...
from app.utils.config import MODEL_DIR, MIN_DATA_DAYS
...
def train_demand_model():
    ...
    if df.empty or len(df) < MIN_DATA_DAYS:
        logger.warning(f"Dados insuficientes para treinar demanda (mínimo {MIN_DATA_DAYS} dias).")
        return None
...
def train_noshow_model():
    """Treina o modelo de detecção de no-show."""
    df = collect_schedules()
    if df.empty or len(df) < 50:
        logger.warning("Dados insuficientes para treinar no-show (mínimo 50 agendamentos).")
        return None
    ...
    if y.nunique() < 2:
        logger.warning("Não há exemplos de no-show suficientes para treinar.")
        return None
